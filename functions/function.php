...
function insert_checkKhoanTuNgayDenNgayXuatKho($a){
	...
	$strcheck = 0;
	if(!empty($datedxuat)){
		if(strtotime($datedxuat) >= strtotime($fromdays) &&  strtotime($datedxuat) <= strtotime($todays))
			$strcheck = 1;
		if(empty($todays) && empty($fromdays))
			$strcheck = 1;
	}
...
function giahuy_thongKeKhoNguonVaoTonKho($cid, $idloaivang, $fromDate, $toDate) {
	...
	$category = $GLOBALS['sp']->getRow($sqlCateg);
	$tableCT = $category['tablect'];
	$tableHachToan =  $category['tablehachtoan'];
		...
		if($idloaivang > 0) {
			...
			if($phieuXuat['id'] > 0) {
				if(empty($whereNhap)) {
					...
					$sqlHaoDuSDDK = "select round(sum(hao),3) as hao, ROUND(SUM(du), 3) as du from $GLOBALS[db_sp].$tableHachToan where typevkc=1 and idloaivang = $idloaivang and dated <= '$dauThangtruoc'";
					...
					$slTon = round((float)$slTonSDDK + (float)$currentTon['slnhapv'] - (float)$currentTon['slxuatv'] - (float)$haoDuDauThang['hao'] + (float)$haoDuDauThang['du'] ,3);
					...
					$sqlSlTonSDDK = "select sltonv from $GLOBALS[db_sp].".$tableHachToan." where typevkc=1 and idloaivang=".$idloaivang." and dated <= '".$dauThangtruoc."' order by id desc limit 1";
					$slTonSDDK = $GLOBALS['sp']->getOne($sqlSlTonSDDK);
					
					$slTonSDDK = round(round(($slTonSDDK - $haoDuSDDK['hao']),3) + $haoDuSDDK['du'],3);

					$sqlHaoDuFromDate = "select ROUND(SUM(hao),3) as hao, ROUND(SUM(du),3) as du from $GLOBALS[db_sp].$tableCT where idloaivang=$idloaivang and typevkc =1 and datedxuat >= '$dateDauThang' and datedxuat < '$fromDate'";
					...
					$sqlHaoDuToDate = "select ROUND(SUM(hao),3) as hao, ROUND(SUM(du),3) as du from $GLOBALS[db_sp].$tableCT where idloaivang=$idloaivang and typevkc =1 and datedxuat >= '$fromDate' and datedxuat <= '$toDate'";
					...
					and dated >= '".$fromDate."'  
					and dated <= '".$toDate."' "; 
					$nhapTndn = $GLOBALS["sp"]->getOne($sqlNhapTndn);
	
					$sqlXuatTndn = "select ROUND(SUM(cannangv), 3) from $GLOBALS[db_sp].".$tableCT." 
					where idloaivang=".$idloaivang." 
					and trangthai = 2
					and typevkc=1
					and datedxuat >= '".$fromDate."'  
...
function giahuy_thongKeTonHienTaiKhoSanXuat($phongbanchuyen, $idloaivang) {
	try{
		$thongke = [];
		$slton = 0;
		$dateDauThang = date('Y').'-'.date('m').'-01';
		$sqlCateg ="select * from $GLOBALS[db_sp].categories where id=$phongbanchuyen";
		$category = $GLOBALS['sp']->getRow($sqlCateg);
		$tableHachToan = $category['tablehachtoan'];

		$sqlCountVang = "select count(*) from $GLOBALS[db_sp].$tableHachToan where idloaivang = $idloaivang";
		$countVang = $GLOBALS['sp']->getRow($sqlCountVang);
		if((int)$countVang > 0) {
			$sqlHaoDu = "select SUM(hao) as hao, SUM(du) as du, SUM(haochenhlech) as haochenhlech, SUM(duchenhlech) as duchenhlech from $GLOBALS[db_sp].$tableHachToan where dated <= '$dateDauThang'";
			$haoDu = $GLOBALS['sp']->getRow($sqlHaoDu);

			$sqlTon = "select * from $GLOBALS[db_sp].$tableHachToan where dated <= '$dateDauThang' order by dated desc limit 1";
			$ton = $GLOBALS['sp']->getRow($sqlTon);

			$slton = round((float)$haoDu['du'] + (float)$ton['slton'] - (float)$haoDu['hao'], 3);
			$slton += round((float)$haoDu['duchecnhlech'] - (float)$haoDu['duchenhlech'], 3);
...
function giahuy_thongKeTonKhoSanXuat($cid, $idloaivang, $fromDate, $toDate) {
	$sltonsddk = 0;
	$slton = 0;
	$thongKe = [];
	$dateDauThang = $dateThangTruoc = null;
	...
	try{
		...
		if(!empty($tableCt) && !empty($tableHachToan)) {
			if(!empty($fromDate)) {
				...
				$dateDauThang = $fromDateExplode[2].'-'.$fromDateExplode[1].'-01';
				$fromDate = $fromDateExplode[2].'-'.$fromDateExplode[1].'-'.$fromDateExplode[0];
			}
			if(!empty($toDate)) {
				$toDateExplode = explode('/', $toDate);
				$toDate = $toDateExplode[2].'-'.$toDateExplode[1].'-'.$toDateExplode[0];
			}

			if((int)$idloaivang > 0) {
				$sqlCountVang = "select count(*) from $GLOBALS[db_sp].$tableHachToan where idloaivang=$idloaivang";
				$countVang = $GLOBALS['sp']->getOne($sqlCountVang);
				if((int)$countVang > 0) {
					if(empty($fromDate) && empty($toDate)) {
						$dateDauThang = date("Y").'-'.date('m').'-01';
						$dateThangTruoc = strtotime(date('Y-m-d', strtotime($dateDauThang)).' - 1 month');
						$dateThangTruoc = date('Y-m-d',$dateThangTruoc);

						$sqlHaoDuDk = "select ROUND(SUM(hao),3) as hao, ROUND(SUM(du),3) as du, ROUND(SUM(haochenhlech),3) as haochenhlech, ROUND(SUM(duchenhlech),3) as duchenhlech 
						from $GLOBALS[db_sp].$tableHachToan where idloaivang=$idloaivang and dated <= '$dateThangTruoc'";
						$haoDuDk = $GLOBALS['sp']->getRow($sqlHaoDuDk);
					
						$sqlTonDk = "select sltonv from $GLOBALS[db_sp].$tableHachToan where idloaivang=$idloaivang and dated <= '$dateThangTruoc' order by dated desc limit 1";
						$tonDk = $GLOBALS['sp']->getOne($sqlTonDk);

						$sltonsddk = round((float)$tonDk + (float)$haoDuDk['du'] - (float)$haoDuDk['hao'],3);
						$sltonsddk = round((float)$sltonsddk + (float)$haoDuDk['haochenhlech'] - (float)$haoDuDk['duchenhlech'],3);

						$sqlHaoDuDateDauThang = "select ROUND(SUM(hao),3) as hao, ROUND(SUM(du),3) as du, ROUND(SUM(haochenhlech),3) as haochenhlech, ROUND(SUM(duchenhlech),3) as duchenhlech 
						...
						$thongKe['slton'] = $slton;
						$thongKe['hao'] = $haoDuDateDauThang['hao'];
						$thongKe['du'] = $haoDuDateDauThang['du'];
						$thongKe['slhaochenhlech'] = $haoDuDateDauThang['slhaochenhlech'];
						$thongKe['slduchenhlech'] = $haoDuDateDauThang['slduchenhlech'];
						$thongKe['slnhap'] = $currentTon['slnhapv'];
						$thongKe['slxuat'] = $currentTon['slxuatv'];
					} else {
						$dateThangTruoc = strtotime(date('Y-m-d', strtotime($dateDauThang)).' - 1 month');
						$dateThangTruoc = date('Y-m-d',$dateThangTruoc);
						...
						$sltonsddk = round((int)$tonDk + (int)$haoDuDk['du'] - (int)$haoDuDk['hao'],3);
						$sltonsddk = round((int)$sltonsddk + (int)$haoDuDk['haochenhlech'] - (int)$haoDuDk['duchenhlech'],3);
						///
						$sqlHaoDuFromDate = "select ROUND(SUM(hao),3) as hao, ROUND(SUM(du),3) as du, ROUND(SUM(haochenhlech),3) as haochenhlech, ROUND(SUM(duchenhlech),3) as duchenhlech 
						from $GLOBALS[db_sp].$tableHachToan where idloaivang=$idloaivang and dated >= '$dateDauThang' and dated < '$fromDate'";
						$haoDuFromDate = $GLOBALS['sp']->getRow($sqlHaoDuFromDate);

						$sqlNhapFromDate = "select ROUND(SUM(cannangv), 3) from $GLOBALS[db_sp].$tableCt where idloaivang=$idloaivang and type=1 and typechuyen=2 and dated >= '$dateDauThang' and dated < '$fromDate'";
						$nhapFromDate = $GLOBALS['sp']->getOne($sqlNhapFromDate);

						$sqlXuatFromDate = "select ROUND(SUM(cannangv), 3) from $GLOBALS[db_sp].$tableCt where idloaivang=$idloaivang and type in (2,3) and datedxuat >= '$dateDauThang' and datedxuat < '$fromDate'";
						$xuatFromDate = $GLOBALS['sp']->getOne($sqlXuatFromDate);

						$slTonFromDate = round((float)$nhapFromDate + (float)$haoDuFromDate['du'] - (float)$xuatFromDate - (float)$haoDuFromDate['hao'], 3);
						$slTonFromDate += round((float)$haoDuFromDate['duchenhlech'] - (float)$haoDuFromDate['haochenhlech'], 3);
						$sltonsddk += round((float)$slTonFromDate, 3);
						
						///
						$sqlHaoDuToDate = "select ROUND(SUM(hao),3) as hao, ROUND(SUM(du),3) as du, ROUND(SUM(haochenhlech),3) as haochenhlech, ROUND(SUM(duchenhlech),3) as duchenhlech 
						from $GLOBALS[db_sp].$tableHachToan where idloaivang=$idloaivang and dated >= '$fromDate' and dated <= '$toDate'";
						$haoDuToDate = $GLOBALS['sp']->getRow($sqlHaoDuToDate);

						$sqlNhapToDate = "select ROUND(SUM(cannangv), 3) from $GLOBALS[db_sp].$tableCt where idloaivang=$idloaivang and type=1 and typechuyen=2 and dated >= '$fromDate' and dated <= '$toDate'";
						$nhapToDate = $GLOBALS['sp']->getOne($sqlNhapToDate);

						$sqlXuatToDate = "select ROUND(SUM(cannangv), 3) from $GLOBALS[db_sp].$tableCt where idloaivang=$idloaivang and type in (2,3) and datedxuat >= '$fromDate' and datedxuat <= '$toDate'";
						$xuatToDate = $GLOBALS['sp']->getOne($sqlXuatToDate);
						
						$slTonToDate = round((float)$nhapToDate + (float)$haoDuToDate['du'] - (float)$xuatToDate - (float)$haoDuToDate['hao'], 3);
						$slTonToDate += round((float)$haoDuToDate['duchenhlech'] - (float)$haoDuToDate['haochenhlech'], 3);
						$slton = $slTonToDate + $sltonsddk;

						$thongKe['slton'] = $slton;
						$thongKe['sltonsddk'] = $sltonsddk;
						$thongKe['slhao'] = $haoDuToDate['hao'];
						$thongKe['sldu'] = $haoDuToDate['du'];
						$thongKe['slhaochenhlech'] = $haoDuToDate['slhaochenhlech'];
						$thongKe['slduchenhlech'] = $haoDuToDate['slduchenhlech'];
						$thongKe['slnhap'] = $nhapToDate;
...
function giahuy_ghiSoHachToan($tablehachtoan, $tablenhan, $id, $typehachtoan) {
	...
	$slnhapkimcuongrc = $dongianhap= 0;
	$slxuatkimcuongrc = $dongiaxuat = 0;
	...
	$sqlHachToan = "select * from $GLOBALS[db_sp].$tablehachtoan where dated = '".$dateDauThang."' and idloaivang= ".$phieu['idloaivang']." and $whereTypevkc";
...
function giahuy_ghiSoHachToanKhoSanXuat($tablehachtoan, $tablenhan, $id) {
	...
	$slnhapvh = $slnhapv = $slnhaph = 0;
	$slxuatvh = $slxuatv = $slxuath = 0;
	$slnhapkimcuong = $slxuatkimcuong = 0;
	$dongianhap = $dongiaxuat = 0;
	...
	if ((int)$phieu['typevkc'] == 1) {
		$whereSql = "idloaivang=".$phieu['idloaivang']." and typevkc = 1";
	} else {
		$whereSql = "typevkc = 2";
	}

	$currentHachToan = [];
	$sqlHachToanThisMonth = "select * from $GLOBALS[db_sp].$tablehachtoan where $whereSql and dated='$dateDauThang'";
	$hachToanThisMonth = $GLOBALS['sp']->getRow($sqlHachToanThisMonth);
	if(empty($hachToanThisMonth['id'])) {
		$sqlHachToanLastMonth = "select * from $GLOBALS[db_sp].$tablehachtoan where $whereSql order by dated desc limit 1";
		$hachToanLastMonth = $GLOBALS['sp']->getRow($sqlHachToanLastMonth);
		if ((int)$phieu['typevkc'] == 1) {
			$currentHachToan['slnhapvh'] = $slnhapvh;
			$currentHachToan['slxuatvh'] = $slxuatvh;
			...
			$currentHachToan['slnhapkimcuong'] = $slnhapkimcuong;
			$currentHachToan['slxuatkimcuong'] = $slxuatkimcuong;
			$currentHachToan['sltonkimcuong'] = round((float)$hachToanLastMonth['sltonkimcuong'] + (float)$slnhapkimcuong - (float)$slxuatkimcuong, 3);
		...
		if ((int)$phieu['typevkc'] == 1) {
			...
			$currentHachToan['slnhapkimcuong'] = round((float)$hachToanThisMonth['slnhapkimcuong'] + (float)$slnhapkimcuong, 3);
			$currentHachToan['slxuatkimcuong'] = round((float)$hachToanThisMonth['slxuatkimcuong'] + (float)$slxuatkimcuong, 3);
			$currentHachToan['sltonkimcuong'] = round((float)$hachToanThisMonth['sltonkimcuong'] + (float)$slnhapkimcuong - (float)$slxuatkimcuong, 3);
			$currentHachToan['dongianhap'] = round((float)$hachToanThisMonth['dongianhap'] + (float)$dongianhap, 3);
			$currentHachToan['dongiaxuat'] = round((float)$hachToanThisMonth['dongiaxuat'] + (float)$dongiaxuat, 3);
			$currentHachToan['tongdongia'] = round((float)$hachToanThisMonth['tongdongia'] + (float)$dongianhap - (float)$dongiaxuat, 3);
		}
...
function dieuChinhSoLieuHachToanKhoNguonVao($table,$tablehachtoan,$idloaivang) {
	...
	for($i=ceil(count($rs))-1; $i >= 0; $i--) {
		...
		$rsTongNhapTrongThang = $GLOBALS['sp']->getRow($sqlTongNhapTrongThang);	
		var_dump($rsTongNhapTrongThang);die();
		// TÍNH TỔNG SỐ LƯỢNG XUẤT DỰA TRÊN CÁC PHIẾU XUẤT ỨNG VỚI IDLOAIVANG ĐANG XỬ LÝ
		$sqlTongXuatTrongThang = "select ROUND(SUM(cannangvh), 3) as cannangvh, 
										ROUND(SUM(cannangh), 3) as cannangh, 
										ROUND(SUM(cannangv), 3) as cannangv
		...
		// Nếu trong tháng có nhập hoặc xuất idloaivang này thì mới tiến hành so sánh và xử lý
		if((float)$rsTongNhapTrongThang['cannangv'] != 0 || (float)$rsTongXuatTrongThang['cannangv'] != 0) {
			...
			$slnhapvSDDK = $GLOBALS['sp']->getOne($sqlNhapSDDKTrongThang);
			// So sánh số lượng nhập vàng trong sodudauky với tổng số lượng nhập của các phiếu cộng lại
			if((float)$rsTongNhapTrongThang['cannangv'] != (int)$slnhapvSDDK){
				$arrUpdateNX['slnhapv'] = $rsTongNhapTrongThang['cannangv'];
				$arrUpdateNX['slnhaph'] = $rsTongNhapTrongThang['cannangh'];
				$arrUpdateNX['slnhapvh'] = $rsTongNhapTrongThang['cannangvh'];
				$checkSaiSoNX = 1;
			...
			$slxuatvSDDK = $GLOBALS['sp']->getOne($sqlXuatSDDKTrongThang);
			// So sánh số lượng nhập vàng trong sodudauky với tổng số lượng nhập của các phiếu cộng lại
			if((float)$rsTongXuatTrongThang['cannangv'] != (int)$slxuatvSDDK){
				$arrUpdateNX['slxuatv'] = $rsTongXuatTrongThang['cannangv'];
				$arrUpdateNX['slxuath'] = $rsTongXuatTrongThang['cannangh'];
				$arrUpdateNX['slxuatvh'] = $rsTongXuatTrongThang['cannangvh'];
				$checkSaiSoNX = 1;
			...
			if($checkSaiSoNX == 1) {
				...
				// Lấy ra số lượng tồn của tháng trước có hạch toán
				$sqlGetSLTonThangTruoc = "select sltonv, sltonh, sltonvh from $GLOBALS[db_sp].".$tablehachtoan." where idloaivang=".$idloaivang." and typevkc=1 and dated < '".$dateDauThang."' order by dated desc limit 1";
				$rsGetSLTonThangTruoc = $GLOBALS['sp']->getRow($sqlGetSLTonThangTruoc);
				// soluongton(trong tháng) = soluongton(tháng trước) + soluongnhap(trong tháng) - soluongxuat(trong tháng)
				$sltonvTrongThang = round(round(($rsGetSLTonThangTruoc['sltonv'] + $rsTongNhapTrongThang['cannangv']),3) - $rsTongXuatTrongThang['cannangv'],3);
				$sltonhTrongThang = round(round(($rsGetSLTonThangTruoc['sltonh'] + $rsTongNhapTrongThang['cannangh']),3) - $rsTongXuatTrongThang['cannangh'],3);
				$sltonvhTrongThang = round(round(($rsGetSLTonThangTruoc['sltonvh'] + $rsTongNhapTrongThang['cannangvh']),3) - $rsTongXuatTrongThang['cannangvh'],3);
				//var_dump($rsGetSLTonThangTruoc['sltonvh'] .'+'. $rsTong0NhapTrongThang['cannangvh'] .'-'. $rsTongXuatTrongThang['cannangvh']);die();
				$arrUpdateNX['sltonv'] = $sltonvTrongThang;
				$arrUpdateNX['sltonh'] = $sltonhTrongThang;
				$arrUpdateNX['sltonvh'] = $sltonvhTrongThang;
...
function giahuy_dieuChinhSoLieuHachToanKhoNguonVao($table,$tablehachtoan,$idloaivang) {
	$sqlHachToan = "select * from $GLOBALS[db_sp].$tablehachtoan where idloaivang=$idloaivang and typevkc = 1 order by dated desc limit 3";
	...
	for($i=$hachToanLenght-1; $i>=0; $i--) {
		...
		$sqlTongNhap = "select ROUND(SUM(cannangvh), 3) as cannangvh, 
					ROUND(SUM(cannangh), 3) as cannangh, 
					ROUND(SUM(cannangv), 3) as cannangv
					from $GLOBALS[db_sp].$table where idloaivang=$idloaivang and type=2 and typevkc = 1 and dated >= '$dateDauThang' and dated <= '$dateCuoiThang'";
		$tongNhap = $GLOBALS['sp']->getRow($sqlTongNhap);

		$sqlTongXuat = "select ROUND(SUM(cannangvh), 3) as cannangvh, 
					ROUND(SUM(cannangh), 3) as cannangh, 
					ROUND(SUM(cannangv), 3) as cannangv
					from $GLOBALS[db_sp].$table where idloaivang=$idloaivang and type=2 and trangthai = 2 and typevkc = 1 and datedxuat >= '$dateDauThang' and datedxuat <= '$dateCuoiThang'";
		...
		if((float)$tongNhap['cannangv'] != (float)$hachToan[$i]['slnhapv']) {
			...
			$hachToanUpdate['slnhapv'] = $tongNhap['cannangv'];
		}

		if((float)$tongXuat['cannangv'] != (float)$hachToan[$i]['slxuatv']) {
			$hachToanUpdate['slxuatvh'] = $tongXuat['cannangvh'];
			$hachToanUpdate['slxuath'] = $tongXuat['cannangh'];
			$hachToanUpdate['slxuatv'] = $tongXuat['cannangv'];
		}

		$sqlSlTonThangTruoc = "select sltonvh, sltonh, sltonv from $GLOBALS[db_sp].$tablehachtoan where idloaivang = $idloaivang and typevkc = 1 and dated < '$dateDauThang' order by dated desc limit 1";
		$slTonThangTruoc = $GLOBALS['sp']->getRow($sqlSlTonThangTruoc); 
...
function giahuy_dieuChinhSoLieuHachToanKhoNuTrangTraVe($table,$tablehachtoan,$idloaivang) {
	...
	for($i=$hachToanLenght-1; $i>=0; $i--) {
		...
		$sqlTongNhap = "select ROUND(SUM(cannangvh), 3) as cannangvh, 
					ROUND(SUM(cannangh), 3) as cannangh, 
					ROUND(SUM(cannangv), 3) as cannangv
					from $GLOBALS[db_sp].$table where idloaivang=$idloaivang and type=2 and datednhap >= '$dateDauThang' and datednhap <= '$dateCuoiThang'";
		$tongNhap = $GLOBALS['sp']->getRow($sqlTongNhap);

		$sqlTongXuat = "select ROUND(SUM(cannangvh), 3) as cannangvh, 
					ROUND(SUM(cannangh), 3) as cannangh, 
					ROUND(SUM(cannangv), 3) as cannangv
					from $GLOBALS[db_sp].$table where idloaivang=$idloaivang and type=2 and trangthai = 2 and datedxuat >= '$dateDauThang' and datedxuat <= '$dateCuoiThang'";
		$tongXuat = $GLOBALS['sp']->getRow($sqlTongXuat);
		...
		// dd($hachToanUpdate);
		// dd((float)$tongXuat['cannangv'] .'-'. (float)$hachToan[$i]['slxuatv']);
		$sqlSlTonThangTruoc = "select sltonvh, sltonh, sltonv from $GLOBALS[db_sp].$tablehachtoan where idloaivang = $idloaivang and dated < '$dateDauThang' order by dated desc limit 1";
		$slTonThangTruoc = $GLOBALS['sp']->getRow($sqlSlTonThangTruoc); 
...
function giahuy_dieuChinhSoLieuHachToanKhoSanXuat($table,$tablehachtoan,$idloaivang) {
	...
	for($i=$hachToanLenght-1; $i>=0; $i--) {
		...
		$sqlTongNhap = "select ROUND(SUM(cannangvh), 3) as cannangvh, 
					ROUND(SUM(cannangh), 3) as cannangh, 
					ROUND(SUM(cannangv), 3) as cannangv
					from $GLOBALS[db_sp].$table where idloaivang=$idloaivang and type=1 and typechuyen=2 and dated >= '$dateDauThang' and dated <= '$dateCuoiThang'";
		$tongNhap = $GLOBALS['sp']->getRow($sqlTongNhap);
...
function GetNameFromSelect($table, $names, $select){
	$name = '';
	if($select != ''){
		$sql = "select ".$names." from $GLOBALS[db_sp].".$table." where id= ($select)";
		$name = $GLOBALS["sp"]->getOne($sql);
	}
	return $name;
}
function ThongKeKhoNuTrangTraVe ($cid, $idloaivang, $fromDate, $toDate) {
	$sqlCateg = "select * from $GLOBALS[db_sp].categories where id = $cid";
	$category = $GLOBALS['sp']->getRow($sqlCateg);
	$tableCt= $category['tablect'];
	$tableHachToan = $category['tablehachtoan'];
	$thongKe = [];
	if (!empty($category['tablect']) && !empty($category['tablehachtoan'])) {
		if (!empty($fromDate)) {
		...
		if ($idloaivang >= 0) {
			$sqlCheckVang = "select id from $GLOBALS[db_sp].$tableCt where idloaivang = $idloaivang limit 1";
			$checkVang = $GLOBALS['sp']->getOne($sqlCheckVang);
			if (ceil($checkVang) > 0) {
				if (empty($fromDate)) {
					$dateDauThang = date('Y-m-01');
					$dateThangTruoc = date('Y-m-01', strtotime("$dateDauThang -1 month"));
					$sqlHaoDuSddk = "select ROUND(SUM(hao), 3), ROUND(SUM(du), 3) from $GLOBALS[db_sp].$tableHachToan where idloaivang = $idloaivang and dated <= '$dateThangTruoc'";
					$haoDuSddk = $GLOBALS['sp']->getRow($sqlHaoDuSddk);
					$sqlTonSddk = "select sltonv from $GLOBALS[db_sp].$tableHachToan where idloaivang = $idloaivang and dated <= '$dateThangTruoc' order by dated desc limit 1";
					$tonSddk = $GLOBALS['sp']->getOne($sqlTonSddk);

					$tonDk = round(($tonSddk - $haoDuSddk['hao']), 3) + $haoDuSddk['du'];
					$thongKe['sltonsddk'] = $tonDk;

					$sqlHaoDuHienTai = "select ROUND(SUM(hao), 3), ROUND(SUM(du), 3) from $GLOBALS[db_sp].$tableHachToan where idloaivang = $idloaivang and dated <= '$dateDauThang'";
					$haoDuHienTai = $GLOBALS['sp']->getRow($sqlHaoDuHienTai);
					$sqlTonHienTai = "select * from $GLOBALS[db_sp].$tableHachToan where idloaivang = $idloaivang and dated = '$dateDauThang'";
					$tonHienTai = $GLOBALS['sp']->getRow($sqlTonHienTai);

					$tonHienTai = round(($tonHienTai['sltonv'] - $haoDuHienTai['hao']),3) + $haoDuHienTai['du'] + $tonDk;
					$thongKe['slton'] = $tonHienTai;
					$thongKe['slnhap'] = $haoDuHienTai['slnhapv'];
					$thongKe['slxuat'] = $haoDuHienTai['slxuatv'];
				}
				$thongKe['idloaivang'] = $idloaivang;
			}
		}
