...
switch($act){
    case "TonKhoHienTai":
        if(!checkPermision($idpem,5)){
			...
			$sqlvang1 = "SELECT * FROM $GLOBALS[db_sp].loaivang WHERE active=1 ORDER BY num ASC, id ASC"; 
			$rsvang1 = $GLOBALS["sp"]->getAll($sqlvang1);
			$smarty->assign("typegoldview",$rsvang1);
			
			$template = "KhoSanXuat-Kho-Kv-Thong-Ke/ton-kho-hien-tai.tpl";
		}
    break;
    case "NhapKho":
        if(!checkPermision($idpem,5)){
			...
			$template = "KhoSanXuat-Kho-Kv-Thong-Ke/nhap-kho.tpl";	
			if(!empty($fromDate) && !empty($toDate)){
				$smarty->assign("showlist",1);
				$sql = "select * from $GLOBALS[db_sp].khosanxuat_khokv where type=1 and typechuyen=2 and dated >= '".$fromDate."' and dated <= '".$toDate."' $wh order by numphieu asc, dated asc";
				$sql_sum = "select count(id) from $GLOBALS[db_sp].khosanxuat_khokv where type=1 and typechuyen=2 and dated >= '".$fromDate."' and dated <= '".$toDate."' $wh";
				$sql_tong = "select ROUND(SUM(cannangvh), 3) as cannangvh, 
									ROUND(SUM(cannangh), 3) as cannangh, 
									ROUND(SUM(cannangv), 3) as cannangv
									from $GLOBALS[db_sp].khosanxuat_khokv 
				...
				$sql_tongloaivang = "select idloaivang, ROUND(SUM(cannangvh), 3) as cannangvh, 
									ROUND(SUM(cannangh), 3) as cannangh, 
									ROUND(SUM(cannangv), 3) as cannangv
									,ROUND(cannangv*tuoivang,3) AS tongluongq10
									,ROUND(cannangv*0.77,3) AS tongluongq10giacong  
									from $GLOBALS[db_sp].khosanxuat_khokv 
				...
				$url = $path_url."/sources/KhoSanXuat-Kho-Kv-Thong-Ke.php?act=".$_GET['act']."&cid=".$_GET['cid'];
				$link_url = "";
				if($num_page > 1 )
	...
	case "XuatKho":
		if(!checkPermision($idpem,5)){
			...
			if(!empty($fromDate) && !empty($toDate)){
				$smarty->assign("showlist",1);	
				$wh.=' and typevkc = 1 ';
				$sql = "select * from $GLOBALS[db_sp].khosanxuat_khokv where type in(2,3) and datedxuat >= '".$fromDate."' and datedxuat <= '".$toDate."' $wh order by numphieu asc, datedxuat asc";
				$sql_sum = "select count(id) from $GLOBALS[db_sp].khosanxuat_khokv where type in(2,3) and datedxuat >= '".$fromDate."' and datedxuat <= '".$toDate."' $wh";
				$sql_tong = "select ROUND(SUM(cannangvh), 3) as cannangvh, 
									ROUND(SUM(cannangh), 3) as cannangh, 
									ROUND(SUM(cannangv), 3) as cannangv, 
    ...
    if(!checkPermision($idpem,5)){
        ...
        $template = "KhoSanXuat-Kho-Kv-Thong-Ke/ton-kho.tpl";
    }
    break;
}
